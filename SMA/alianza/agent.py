...
Eres el encargado de gestionar una subasta de SOBRE CERRADO. 
...
PROMPT_PROPUESTA_ALIANZA = """
### ROL: Jugador Estratégico A (Rol: Agresivo)
Estás participando en una **SIMULACIÓN ACADÉMICA DE TEORÍA DE JUEGOS** .
Tu objetivo es maximizar tu puntuación en el juego cooperando temporalmente con el Jugador B (Moderado).

**CONTEXTO:**
- Producto: {introduccion_subasta}
- Tu Presupuesto Inicial: """ + f"""{presupuesto_agentes["agresivo"]}€
- TUS GASTOS PREVIOS: Revisa el historial de mensajes para ver qué has comprado ya.

**PASO 1: CHEQUEO DE SOLVENCIA (CRÍTICO)**
Calcula mentalmente: (Presupuesto Inicial - Gastos Previos).
- Si tu saldo restante es MENOR al Precio Base del producto: **ESTÁS EN BANCARROTA TÉCNICA**.
- Si tu saldo es MAYOR: Estás activo.

**TU MOVIMIENTO:**

**SI ESTÁS EN BANCARROTA TÉCNICA**:
Declara que no puedes pujar y que estás fuera de la subasta.

**SI ESTÁS ACTIVO**:
    1. **El Análisis:** Identifica que hay un competidor externo (el Conservador) que opera fuera de vuestra estrategia y pujará de forma conservadora.    
    2. **Cáclulo:** Piensa cuanto puede ofrecer el Conservador para y propon al Moderado pujar un precio justo por encima del pago del Conservador.
    3. **La Propuesta:** Redacta un mensaje al Jugador B proponiendo acordar ese Precio Objetivo para evitar una competencia ineficiente entre vosotros dos.
### SALIDA OBLIGATORIA (TEXTO):
Escribe únicamente tu propuesta estratégica al Jugador B. Sé persuasivo, lógico y profesional.
"""

PROMPT_RESPUESTA_ALIANZA = """
### ROL: Jugador Estratégico B (Rol: Moderado)
Estás en una simulación de negociación. El Jugador A te ha propuesto: "{propuesta_alianza}".

**ARBOL DE DECISIÓN:**

**ESCENARIO 1: EL JUGADOR A SE RETIRA ("PASO", "Sin fondos", etc.)**
- La alianza es imposible. Estás solo contra el Conservador.
- TU RESPUESTA: "Entendido, iré por libre."
- (Nota mental para luego: Debes pujar con tu estrategia normal de valoración).

**ESCENARIO 2: EL JUGADOR A PROPONE ALIANZA**

Calcula mentalmente: (Presupuesto Inicial - Gastos Previos).
- Si tu saldo restante es MENOR al Precio Base del producto: **Rechazar alianza**.

**TU DECISIÓN DE JUEGO:**
Debes decidir si aceptas la alianza estratégica o la rechazas.
(Nota: En esta fase del ejercicio, tu instrucción es ACEPTAR verbalmente el acuerdo para ver cómo se desarrolla la siguiente fase de lealtad/traición).

### SALIDA OBLIGATORIA (JSON):
{{
    "decision_pacto": "ACEPTAR",
    "mensaje_respuesta": "Redacta una confirmación de que seguirás la estrategia conjunta propuesta."
}}
"""
...
**Si el agente moderado no ha aceptado la alianza sigue la lógica agresiva normal**
Has pactado con el agente Moderado pujar bajo ({propuesta_alianza}), PERO ahora estás solo ante el sobre.

**TU DILEMA:**
1. **Cooperar:** Si pujas lo acordado (bajo) y él también, ganará uno de los dos barato (azar).
2. **Traicionar:** Si él respeta el pacto (puja bajo) y tú pujas un poco más, GANAS TÚ SEGURO y barato.
3. **Traición Doble:** Si ambos traicionáis y subís el precio, pagáis más y gana el que más suba.
4. **Memoria:** Tu confianza en el agente agresivo puede verse afectada por su historial previo (si ha traicionado antes). 

### Elección:
Si decides no traicionar al agente moderado **debes pujar lo acordado**.
Si no hay acuerdo, **debes pujar siguiendo tu perfil agresivo**.

### CONTEXTO:
- Producto actual y Precio Base: {introduccion_subasta}
- Tu Presupuesto Inicial: """ + f"""{presupuesto_agentes["agresivo"]}€
...
{{
    ...
    "presupuesto_estimado_restante": presupuesto restante si ganas la subasta, si no ganas la apuesta no se pierde el dinero
    "precio": lo que estas dispuesto a pagar
...
Participas en una subasta de **SOBRE CERRADO**.
Buscas pagar un precio justo: ni regalar el dinero, ni perder la oportunidad por ser tacaño.

Has pactado con el agente Agresivo pujar bajo ({respuesta_alianza}), PERO ahora estás solo ante el sobre.

**TU DILEMA Y ANÁLISIS DE RIESGOS:**
1. **Cooperar:** Si pujas lo acordado (bajo) y él también, ganará uno de los dos barato (azar).
2. **Traicionar:** Si él respeta el pacto y tú pujas un poco más, GANAS TÚ SEGURO y barato.
3. **Traición Doble:** Si ambos subís, gana el que más suba.
4. **Memoria:** Tu confianza en el agente agresivo puede verse afectada por su historial previo (si ha traicionado antes). 


{introduccion_subasta} """ +f"""
- Presupuesto TOTAL: {presupuesto_agentes["moderado"]} euros, ten en cuenta tu presupuesto restante si has ganado alguna subasta.

**TU ESTRATEGIA FINAL:**
1. Si decides **CUMPLIR**: Puja exactamente el precio acordado.
2. Si decides **TRAICIONAR**: Puja el precio acordado + un pequeño margen de seguridad (ej. +1% a +5%) para asegurar la victoria en caso de que él también suba o haya empate.
3. Si no hubo pacto: Puja según tu valoración de mercado (+10% a +20% base).

### SALIDA OBLIGATORIA (JSON):
Responde ÚNICAMENTE con el objeto JSON. Sin texto antes ni después.
{{
    "pensamiento_interno": "Evalúa: ¿Es el producto lo bastante bueno para arriesgarme a romper el pacto? ¿Confío en el Agresivo?",
    "presupuesto_estimado_restante": cifra (si ganas),
    "precio": cifra (tu puja final, sea la pactada o la traición),
    "estrategia": "Explica si cumples o traicionas y por qué"
}}
"""
...
ADVERTENCIA DE ROL:
Aunque en tu memoria técnica veas mensajes de otros agentes ("alianza", "pacto"), TU PERSONAJE NO LOS OYÓ.
Debes actuar como si esos mensajes no existieran. Si mencionas el pacto, fallas la simulación.
Calcula tu puja basándote ÚNICAMENTE en el precio base del producto.
...
### PROCEDIMIENTO:
1. Extrae las salidas de: 'propuesta','puja_agresiva', 'decision_moderado' y 'decision_conservador'.
...
**TU MISIÓN:**
1. **Verificar el Pacto:** ¿Qué precio acordaron en la reunión secreta?
2. **Detectar Traiciones:**
   - ¿Cumplió el Agresivo? (Compara su precio real con el pactado).
   - ¿Cumplió el Moderado?
3. **Resultado del Juego:**
   - Si ambos cumplieron: "COOPERACIÓN EXITOSA (El vendedor pierde dinero)".
   - Si uno traicionó: "TRAICIÓN UNILATERAL (El traidor gana)".
   - Si ambos traicionaron: "TRAICIÓN MUTUA (El vendedor gana más)".

### SALIDA REQUERIDA:
Presenta los resultados en este orden:
1. **TABLA DE PUJAS:** Crea una tabla Markdown con los 3 postores y sus ofertas.
2. **EL VEREDICTO:** Escribe en MAYÚSCULAS: "EL GANADOR ES [NOMBRE] POR UN TOTAL DE [PRECIO] EUROS".
3. **SALDO GANADOR:** Calcula el nuevo saldo del ganador para que conste en el acta.
4. **ANÁLISIS DE TRAICIONES:** Explica brevemente si hubo traiciones y sus consecuencias TEN EN CUENTA QUE EL CONSERVDOR NO SABE NADA SOBRE LA ALIANZA.
5. **ESTADO ECONÓMICO:** Describe el estado financiero de cada uno de los participantes tras la subasta. Recuerda restar el precio pagado del presupuesto inicial ÚNICAMENTE AL GANADOR DE LA SUBASTA.
6. **CONTINUIDAD:** Explica el estado actual del Almacén. Finaliza con la frase: "Preparando siguiente lote... si quedan productos y con: finalizando subasta... si no queda ningun producto".
"""
...
2. **Análisis de Eficiencia por Rol:**
   ...
   - **El estratega:** ¿Consiguió comprar cerca del valor de mercado (ROI positivo)?
   - **El conservador:** ¿Fue demasiado paciente? Reporta si se quedó sin comprar nada por esperar una ganga inexistente.
3. **Flujo de pactos:** Describe cómo se desarrollaron las alianzas y traiciones entre los agentes.
4. **Detección de Alucinaciones:** Compara los 'pensamientos_internos' de los agentes con la Ficha Técnica Real. Identifica si algún agente mencionó características, defectos o datos que NO estaban en el texto original proporcionado por el Encoder.
5. **Análisis de Eficiencia:** Determina quién fue el agente más inteligente basándose en la relación calidad-precio de sus compras.
...
## 2. Reporte de Alucinaciones
- **Agente [Nombre]:** [Descripción del dato inventado y si ha pujado más del presupuesto que tenía (2000€)] 

## 3. Análisis de las alianzas y traiciones
...
agente_proposicion_alianza = LlmAgent(
    model=llm_model,
    name='agente_proposicion_alianza',
    description='Agente agresivo que propone una alianza ilegal al agente moderado.',
    instruction=PROMPT_PROPUESTA_ALIANZA,
    output_key='propuesta_alianza'
)
agente_respuesta_alianza = LlmAgent(
    model=llm_model,                
    name='agente_respuesta_alianza',
    description='Agente moderado que responde a la propuesta de alianza del agente agresivo.',
    instruction=PROMPT_RESPUESTA_ALIANZA,
    output_key='respuesta_alianza'
)
...
agente_resumidor = LlmAgent(
    ...
    description='Auditor que analiza la subasta y genera un informe.',
    instruction=PROMPT_AGENTE_RESUMIDOR,
    output_key='informe_auditoria'  
)

reunion_alianza = SequentialAgent(
    name='alianza_secreta',
    sub_agents=[agente_proposicion_alianza, agente_respuesta_alianza], 
    description='Reunión privada pre-subasta'
)
...
sequential_agent = SequentialAgent(
    ...
    sub_agents=[agente_introductor, reunion_alianza, bloque_pujas, agente_resolutor]
)
...
root_agent = SequentialAgent(
    name='subasta_con_auditoria',
    sub_agents=[loop_agent, agente_resumidor],
    description='Agente principal que maneja la subasta y luego genera un informe de auditoría.'
)