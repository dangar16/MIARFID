...
tokenizer = AutoTokenizer.from_pretrained(
    checkpoint,
    model_max_length=max_tok_length,
)

def preprocess_function(batch):
    inputs = [prefix + doc for doc in batch["hypothesis"]]
...
# LoRA config (MODIFICADO PARA T5)
...
def preprocess_test_inference(text_list):
    inputs = [prefix + text for text in text_list]
    return tokenizer(
        inputs,
        padding=True,
        truncation=True,
...
outputs = []
batch_size = 32
hypothesis_list = raw_test["hypothesis"]

from tqdm import tqdm

for i in tqdm(range(0, len(hypothesis_list), batch_size)):
    batch_texts = hypothesis_list[i : i + batch_size]
    
    inputs = preprocess_test_inference(batch_texts).to(lora_model.device)
